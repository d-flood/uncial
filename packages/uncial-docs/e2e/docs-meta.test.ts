...
test('the editor metadata panel edits navGroup/navOrder and a save round-trips them', async ({
	...
	await expect(page.locator('.uncial-cms-editor-page .ProseMirror')).toContainText(
		'Your presentation layer is the editor'
	);

	// The metadata panel lives behind the "Edit document metadata" dropdown.
	const editor = page.locator('.uncial-cms-editor-page');
	await editor.getByLabel('Edit document metadata').click();

	// The schema's meta fields are forwarded through EditorPage, so navGroup and
	// navOrder render as editable fields (not just the pre-existing title).
